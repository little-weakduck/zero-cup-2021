window.addEventListener('scroll', (e) => {
  ...
  if (scrolled <= 0.5) {
    ...
    $title.style.opacity = scrolled
    $PIXAR.style.opacity = scrolled
    $TextBg.style.opacity = scrolled
    $title.style.width = document.documentElement.clientWidth * (1 - scrolled) * (1 - scrolled) * 5 + 'px';
  } else {
    $('#title').css('display', 'block')
    $title.classList.remove('transparent')
    $title.style.opacity = scrolled
    $PIXAR.style.opacity = scrolled
    $title.style.width = document.documentElement.clientWidth * (1 - scrolled) * (1 - scrolled) * 10 + 'px';
  }})